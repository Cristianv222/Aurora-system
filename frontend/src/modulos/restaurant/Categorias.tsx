...
const Categorias: React.FC = () => {
    ...
    if (loading) return <div className="p-6 text-center text-slate-500">Cargando categorías...</div>;
                                ...
                                categories.map(cat => (
                                    <tr key={cat.id} className="hover:bg-slate-50/50 transition">
                                        ...
                                        <td className="px-6 py-4 text-right space-x-2">
                                            <button
                                                className="px-2 py-1.5 border border-slate-200 hover:bg-slate-50 text-slate-600 rounded-lg transition"
                                                onClick={() => handleEditCategory(cat)}
                                            >
                                                ✏️
                                            </button>
                                            <button
                                                className="px-2 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg transition"
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingCategory ? "Editar Categoría" : "Nueva Categoría"}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                        <button type="button" className="px-4 py-2 border border-slate-200 rounded-lg text-sm font-semibold text-slate-600 hover:bg-slate-50 transition" onClick={() => setIsModalOpen(false)}>Cancelar</button>
                        <button type="submit" className="px-4 py-2 bg-slate-900 text-white rounded-lg text-sm font-semibold hover:bg-slate-800 transition">Guardar</button>
                    </div>
                </form>
            </Modal>
        </div >
