...
const BarraLateralRestaurant: React.FC = () => {
    const location = useLocation();

    const isActive = (path: string): string => {
        return location.pathname === path ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200';
    };

    const links = [
        { path: '/restaurant', label: 'Panel Principal', icon: 'bi-grid-fill' },
        { path: '/restaurant/pos', label: 'Punto de Venta', icon: 'bi-shop' },
        { path: '/restaurant/orders', label: 'Órdenes', icon: 'bi-list-ul' },
        { path: '/restaurant/inventory', label: 'Inventario', icon: 'bi-archive-fill' },
        { path: '/restaurant/customers', label: 'Clientes', icon: 'bi-people-fill' },
        { path: '/restaurant/reservations', label: 'Reservaciones', icon: 'bi-calendar-check-fill' },
        { path: '/restaurant/shift', label: 'Caja (Turnos)', icon: 'bi-safe2-fill' },
    ];
    ...
    return (
        <aside className="w-64 bg-slate-900 text-slate-100 flex flex-col h-screen border-r border-slate-800 flex-shrink-0 z-10 relative">
            <div className="p-5 text-lg font-bold border-b border-slate-800 bg-slate-950/80 tracking-wide text-slate-200 flex items-center gap-2">
                <i className="bi bi-award text-slate-400"></i>
                <span>Fortaleza Rest.</span>
            </div>
