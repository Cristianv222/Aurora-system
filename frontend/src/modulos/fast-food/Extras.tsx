...
const Extras: React.FC = () => {
    ...
    const [newExtra, setNewExtra] = useState<ExtraForm>({ name: '', description: '', price: '', image: null });
    ...
    const handleEditExtra = (extra: Extra) => {
        ...
        setNewExtra({ name: extra.name, description: extra.description || '', price: String(extra.price), image: null });
        setIsModalOpen(true);
    };
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if (newExtra.image instanceof File) formData.append('image', newExtra.image);
    ...
    if (loading) return <div className="p-6 text-center text-slate-500 font-semibold text-sm">Cargando extras...</div>;

    return (
        <div className="p-6 max-w-6xl mx-auto space-y-6">
            <div className="flex justify-between items-center border-b border-slate-200 pb-4">
                <h3 className="text-xl font-bold text-slate-800">Gestión de Extras / Adicionales</h3>
                <button
                    ...
                    onClick={() => { setEditingExtra(null); setNewExtra({ name: '', description: '', price: '', image: null }); setIsModalOpen(true); }}
                >
                    + Nuevo Extra
                </button>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse text-sm">
                        ...
                        <tbody className="divide-y divide-slate-100 text-slate-700">
                            {extras.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="px-6 py-8 text-center text-slate-400">No hay extras registrados</td>
                                </tr>
                            ) : (
                                extras.map(extra => (
                                    <tr key={extra.id} className="hover:bg-slate-50/50 transition">
                                        <td className="px-6 py-4 text-xs font-mono text-slate-400">{extra.id}</td>
                                        ...
                                        <td className="px-6 py-4 font-semibold text-emerald-700">${Number(extra.price).toFixed(2)}</td>
                                        <td className="px-6 py-4 text-right space-x-2">
                                            <button className="px-2.5 py-1.5 border border-slate-200 hover:bg-slate-50 text-slate-600 rounded-lg transition" onClick={() => handleEditExtra(extra)}>✏️</button>
                                            <button className="px-2.5 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg transition" onClick={() => handleDeleteExtra(extra.id)}>🗑️</button>
                                        </td>
                                    </tr>
                                ))
                            )}
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingExtra ? 'Editar Extra' : 'Nuevo Extra'}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="flex flex-col gap-1.5">
                        ...
                        <input type="text" name="name" value={newExtra.name} onChange={handleInputChange} required className="w-full border border-slate-200 rounded-xl px-3.5 py-2 text-sm focus:outline-none focus:border-slate-800 transition" />
                    </div>
                    <div className="flex flex-col gap-1.5">
                        <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Descripción</label>
                        <textarea name="description" value={newExtra.description} onChange={handleInputChange} rows={3} className="w-full border border-slate-200 rounded-xl px-3.5 py-2 text-sm focus:outline-none focus:border-slate-800 transition resize-none" />
                    </div>
                    <div className="flex flex-col gap-1.5">
                        <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Precio</label>
                        <input type="number" name="price" value={newExtra.price} onChange={handleInputChange} step="0.01" required className="w-full border border-slate-200 rounded-xl px-3.5 py-2 text-sm focus:outline-none focus:border-slate-800 transition" />
                    </div>
                    <div className="flex flex-col gap-1.5">
                        <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Imagen</label>
                        <input type="file" accept="image/*" onChange={handleImageChange} required={!editingExtra} className="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200" />
                    </div>
