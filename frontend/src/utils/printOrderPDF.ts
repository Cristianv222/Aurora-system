...
export const printOrderPDF = (order: Order | any): void => {
    if (!order) return;
    ...
    yPos += 7;
    doc.text(`Tipo: ${order.order_type_display || 'Normal'}`, 20, yPos);
    ...
    if (order.items && order.items.length > 0) {
        order.items.forEach((item: any) => {
            ...
            doc.text(`$${parseFloat(item.unit_price || item.price).toFixed(2)}`, 145, yPos);
            doc.text(`$${parseFloat(item.line_total || item.subtotal || 0).toFixed(2)}`, 170, yPos, { align: 'right' });
            yPos += 7;
        });
    }
