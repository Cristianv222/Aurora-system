...
class JWTAuthenticationMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        if not auth_header.startswith('Bearer '):
            # Solo para APIs REST, no para admin
        ...
        try:
            response = requests.post(
                ...
                json={'token': token},
                timeout=5,
                headers={'Host': 'auth-service:8000'}  # ← Importante: agregar host correcto
            )
            ...
            logger.error(f"⚠️ Error inesperado en autenticación: {str(e)}")
            if request.path.startswith('/api/'):
                return JsonResponse({
